...
public class SensorController {
	...
	public List<Integer> weightcal(int num) {
		...
		try {
			...
			for(int i=0; i<nationidx.size(); i++) {
				...
				if(!result.contains(nationidx.get(i))) {
					
					result.add(nationidx.get(i));
				}
			}
			return result;
		}
	...
	public @ResponseBody ResponseEntity<Map<String, Object>> selectnation() throws JsonProcessingException {
		...
		try {
			...
			if(nation==null || nation.size() < num*2) {
				List<Nation> nationIdxs = ser.selectNations();
				for(int i=0; i<nationIdxs.size(); i++) {
					Monthtb mon = manser.monthInfo(Integer.parseInt(nationIdxs.get(i).getIdx()));
					List<Image> img = manser.imagesInfo(Integer.parseInt(nationIdxs.get(i).getIdx()));
					List<Route> rou = manser.contentsInfo(Integer.parseInt(nationIdxs.get(i).getIdx()));
					if (mon == null || img == null || rou == null) continue;
					if(result.containsValue(nationIdxs.get(i))) continue;
					nation.add(Integer.parseInt(nationIdxs.get(i).getIdx()));
					if(nation.size()==num*2) break;
				}
			}
			
			System.out.println("안녕하세요. 추천해 드릴 나라의 idx 번호는 다음과 같습니다.");
			if(nation.size()<num*2) {
				System.out.println("고객님? 유감스럽게도 보여드릴 나라가 이것밖에 없는걸 어떡해요.");
				result.put("haeun msg", "죄송한데 나라 부족해서 어쩔 수 없었어요");
			}
